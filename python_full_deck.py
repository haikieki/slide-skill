...
def pil_multiline_c(draw, cx, top, text, fn, fill, lh=None):
    lines = text.split('\n')
    line_h = lh or (fn.size+4 if hasattr(fn,'size') else 20)
    for i, ln in enumerate(lines):
        draw.text((cx, top+i*line_h), ln, font=fn, fill=fill, anchor='mm')
...
def make_step_illust(data, output_path):
    ...
    steps = data['steps']
    n     = len(steps)
    box_w = min(190, (W-40)//n - 14)
    box_h = 230
    gap   = max(28, (W-40-box_w*n)//(n-1)) if n>1 else 0
    total_w = box_w*n + gap*(n-1)
    sx    = (W-total_w)//2

    if data.get('title'):
        d.text((W//2, 16), data['title'], font=pil_font(20), fill=PIL_TEAL, anchor='mm')
    ...
    for i, step in enumerate(steps):
        ...
        # badge
        d.ellipse([bx+box_w//2-24, by+12, bx+box_w//2+24, by+60], fill=PIL_ORANGE)
        pil_text_c(d, bx+box_w//2, by+36, step['num'], pil_font(20), PIL_WHITE)
        # title
        pil_text_c(d, bx+box_w//2, by+80, step.get('title',''), pil_font(15), PIL_TEAL)
        # desc
        pil_multiline_c(d, bx+box_w//2, by+104, step.get('desc',''), pil_font(13), PIL_DARK, lh=18)
        # arrow
        if i < n-1:
            arr_x = bx+box_w+6
            arr_y = H//2 + (10 if data.get('title') else 0)
            pil_arrow_right(d, arr_x, arr_y, gap-12, PIL_TEAL, th=5)
...
def make_comparison_illust(data, output_path):
    ...
    for panel, px, bg, tc in panels:
        pil_rounded_rect(d, [px,pad,px+pw,pad+ph], 18, bg)
        pil_text_c(d, px+pw//2, pad+32, panel['label'], pil_font(22), tc)
        line_y = pad+60
        d.line([(px+20, line_y),(px+pw-20, line_y)], fill=tc if bg==PIL_GRAY else PIL_WHITE, width=2)
        for j, item in enumerate(panel['items']):
            pil_text_c(d, px+pw//2, pad+90+j*62, item, pil_font(17), tc)

    # arrow
    pil_arrow_right(d, pad+pw+8, H//2, pad*2-16, PIL_ORANGE, th=8)
    img.save(output_path)
...
def make_badge_illust(data, output_path):
    ...
    for cx, item in zip(xs, items):
        ...
        pil_text_c(d, cx, cy-16, item.get('icon',''), pil_font(28), PIL_WHITE)
        pil_text_c(d, cx, cy+20, item['value'],       pil_font(20), PIL_WHITE)
        pil_text_c(d, cx, cy+r+26, item['label'],     pil_font(15), PIL_DARK)
...
slides = [
    ...
    {
        "type": "bullets", "page": 3, "theme": THEME,
    ...
    {
        "type": "categories", "page": 4, "theme": THEME,
    ...
    {
        "type": "warnings", "page": 5, "theme": THEME,
...
illustrations = [
    ...
    },
    {
        ...
        "before": {
            "label": "Python なし",
            "items": ["毎日手作業で作業", "データ分析できない", "自動化は夢のまた夢"]
        },
        "after": {
            "label": "Python あり",
...
OUT_DIR = r"C:\Users\haiki\OneDrive\デスクトップ\スキル\スキル開発\スライド\output_Python入門"
